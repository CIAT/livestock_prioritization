...
```{r load WDPA}
WDPA_rast<-terra::rast("Data/WDPA/WDPA_CGIAR_countries.tif")
```
...
```{r plot WPDA, echo =F}
renderPlot({terra::plot(mask(WDPA_rast,CGIAR_countries2),
                        ...
                        axes = F,
                        frame.plot= F)
          terra::plot(CGIAR_countries2,add=T,border=input$BorderCol)
  },height=Plot.Height, width=Plot.Width)
```
...
```{r plot LPS, echo =F}
renderPlot({terra::plot(mask(LPS_Suitable,CGIAR_countries2),
                        main="Global Distribution of LPS (Robinson et al. 2014)",
                        pax=list(cex.axis=input$TextSize),
                        plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
                        col=viridis(n=nrow(terra::cats(LPS_Suitable)[[1]]),option =input$Palette,direction = if(input$Palette=="turbo"){1}else{-1}),
                        cex.main = input$TextSize*1.2,
                        axes = F,
                        frame.plot= F)
          terra::plot(CGIAR_countries2,add=T,border=input$BorderCol)
...
```{r plot TLU, echo =F}
renderPlot({terra::plot(mask(log10(TLU),CGIAR_countries2),
...
:::: {style="display: flex;"}
::: {}
```{r Hazards - CV High Value,echo=F}
  numericInput(inputId="Hazards.CV.High",
             label="Rainfall CV high variability (HCV)",
             value=15,
             min=0,
             max=50,
             step=1,
...
:::
:::{}
```{r Hazards - CV Extreme Value,echo=F}
  numericInput(inputId="Hazards.CV.Ex",
             label="Rainfall CV extremely variable (ExCV)",
             value=30,
             min=0,
             max=50,
             step=1,
...
:::
:::{}
```{r Hazards - Thi Threshold,echo=F}
  numericInput(inputId="Hazards.Thi",
             label="Thermal stress index (ThI)",
             value=79,
             min=0,
             max=200,
             step=1,
...
:::
:::{}
```{r Hazards - Drydays Threshold,echo=F}
  numericInput(inputId="Hazards.Drydays", 
             label="Consecutive drydays (D)",
             value=25,
             min=0,
             max=100,
             step=1,
...
:::
:::{}
```{r Hazards - Floodrisk Threshold,echo=F}
  numericInput(inputId="Hazards.Floodrisk",
             label="Floodrisk (F)",
             value=1,
             min=0,
             max=5,
...
```{r Hazards - read in datasets}
# To characterize climate variability, we used the coefficient of variation of annual mean rainfall, derived from the CHIRPS dataset (Funk et al., 2015).
rain_cv<-terra::resample(terra::rast(list.files("./Data/Hazards","chirps_cv",full.names = T)),BaseRaster,method="bilinear")
# Areas with risk of drought were defined as having more than 25 days without rain per month on average.
drydays<-terra::resample(terra::rast(list.files("./Data/Hazards","dry_days_1981_2019_mean",full.names = T)),BaseRaster,method="bilinear")
# To define areas with risk of flooding we used the UNEP/DEWA/GRID-Europe (2011) dataset, in which flooding risk is ranked from 0 (no risk) to 5 (extreme).  
floodrisk<-terra::resample(terra::rast(list.files("./Data/Hazards","flood_risk_UNEP_wgs84",full.names = T)),BaseRaster,method="near")
# Areas of heat stress were defined as having thermal heat stress (THI, projected for 2030 under an 8.5 RCP scenario) (see Thornton et al., 2021).  
thi<-terra::resample(terra::rast(list.files("./Data/Hazards","thi_max_2030_rcp85",full.names = T)),BaseRaster,method="bilinear")

Hazards<-c(rain_cv,drydays,floodrisk,thi)
names(Hazards)<-c("rain_cv","drydays","floodrisk","thi")
Hazards<-terra::mask(Hazards,CGIAR_countries2)
```

*Raw Hazards*
```{r plot raw hazards, echo =F}
renderPlot({terra::plot(mask(Hazards,CGIAR_countries2),
                        pax=list(cex.axis=input$TextSize),
                        plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
                        col=viridis(n=input$Ncolours,option =input$Palette,direction = if(input$Palette=="turbo"){1}else{-1}),
                        cex.main = input$TextSize*1.2,
...
```{r Hazards - create classification function}
ClassFun<-function(Data,T1,T2,Name){
  if(!(is.null(T1)|is.null(T2))){
    Data[which(Data[]<T1)]<-NA
    Data[which(Data[]>=T2)]<-0
    Data[which(Data[]>=T1 & Data[]<T2)]<-1
  }else{
    Data[which(Data[]<T1)]<-NA
...
```{r Hazards - classify rainfall cv}
rain_cv_class<-reactive({
  ClassFun(Data=Hazards$rain_cv,T1=input$Hazards.CV.High,T2=input$Hazards.CV.Ex,Name="rain_cv")
})
```

```{r Hazards - classify THI}
thi_class<-reactive({
  ClassFun(Data=Hazards$thi,T1=input$Hazards.Thi,T2=NULL,Name="thi")
})
```

```{r Hazards - classify drydays}
drydays_class<-reactive({
  ClassFun(Data=Hazards$drydays,T1=input$Hazards.Drydays,T2=NULL,Name="drydays")
})
```

```{r Hazards - classify floodrisk}
floodrisk_class<-reactive({
  ClassFun(Data=Hazards$floodrisk,T1=input$Hazards.Floodrisk,T2=NULL,Name="floodrisk")
})
```

```{r stack hazard classes}
Hazards_Class<-reactive({c(rain_cv_class(),thi_class(),drydays_class(),floodrisk_class())})
```

*Classified Hazards*
```{r plot hazards, echo=F}
renderPlot({
      terra::plot(x=Hazards_Class(),
                  pax=list(cex.axis=input$TextSize),
                  plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
...
```{r Hazards - Combine Classes}
ClassifyHazards<-function(Data,Threshold,Mask=NULL){
  
  if(!is.null(Mask)){ Data<-mask(Data,Mask)}
  
  # Number of hazards per cell
  Data.N<- terra::app(Data,fun=function(x){sum(!is.na(x))})

  #Reclassify values to create a binary sequence when summed
  # Finding strange results for subst for the below, so went back to basics for rain_cv
  X<-Data$rain_cv
  
  # HCV and ExCV where CV is the only hazard
  X[which(X[]==1)]<-10 # High
  X[which(X[]==0)]<-1 # Extreme
  
  # CV where >1 hazard is present
  X[which(Data.N[]>1 & !is.na(X[]))]<-100
  Data.N[which(Data.N[]==0)]<-NA
  
  Data$rain_cv<-X
  
  Data$thi<-subst(Data$thi,0,1000)
  Data$drydays<-subst(Data$drydays,0,10000)
  Data$floodrisk<-subst(Data$floodrisk,0,100000)
  # Sum layers to create sequence
  Data<- terra::app(Data, sum,na.rm=T)
  
  # Translate binary sequence to label
  labels<-rev(c("ExCV","HCV","CV","Thi","D","F"))
  values<-unique(as.vector(Data))
  X<-unlist(lapply(str_locate_all(str_pad(values, length(labels), pad = "0"),"1"),FUN=function(X){
    paste(rev(labels[unlist(X[,1])]),collapse="+")
    }))
  (labels<-data.table(values=values,labels=X)[order(values)][!is.na(values)][,N:=0:(.N-1)])
  
  # Recode raster to integer values and add labels
  Data<-subst(Data,labels[,values],labels[,N])
  
  # Remove factors below threshold
  X<- table(Data[])
  X<-100*X/sum(X) 
  
  if(!is.null(Threshold)){
    Remove<-as.numeric(names(X[X<Threshold]))
    Data2<-copy(Data)

    if(length(X)>0){
      Keep<-as.numeric(names(X[X>=Threshold]))
  
      Data2<-subst(Data2,Remove,NA)
      Data2<-subst(Data2,Keep,0:(length(Keep)-1))
      
      levels(Data2)<-labels[!N %in% Remove][,N:=0:(length(Keep)-1)][,list(N,labels)]

    }
  

    levels(Data)<-labels[,list(N,labels)]
  
    X<-c(Data,Data2,Data.N)
    names(X)<-c("Complete","Subset","N.Hazards")
...
Hazards_Class2<-reactive({
  ClassifyHazards(Data=Hazards_Class(),Threshold=NULL,Mask=NULL)
})
```
...
renderPlot({
  terra::plot(Hazards_Class2()$Complete,
...
renderPlot({
  terra::plot(Hazards_Class2_Masked()$Complete,
...
renderPlot({
  terra::plot(Hazards_Class2_Masked2(),
...
renderPlot({terra::plot(mask(c(FAO.CH4.Change,FAO.N2O.Change),ExtractBy()),
                        pax=list(cex.axis=input$TextSize),
                        plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
                        col=viridis(n=input$Ncolours,option =input$Palette,direction = if(input$Palette=="turbo"){1}else{-1}),
                        cex.main = input$TextSize*1.2,
...
renderPlot({terra::plot(mask(log10(LS.CO2e()),ExtractBy()),
                        pax=list(cex.axis=input$TextSize),
                        plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
                        range=c(0,8.2),
                        col=viridis(n=input$Ncolours,option =input$Palette,direction = if(input$Palette=="turbo"){1}else{-1}),
...
renderPlot({terra::plot(mask(c(ForestCover09,CarbonStocks,CarbonStocks.Masked),ExtractBy()),
                        pax=list(cex.axis=input$TextSize),
                        plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
                        col=viridis(n=input$Ncolours,option =input$Palette,direction = if(input$Palette=="turbo"){1}else{-1}),
                        cex.main = input$TextSize*1.2,
...
renderPlot({terra::plot(mask(WRI.Pasture.CarbonLoss.ha.yr,ExtractBy()),
...
renderPlot({terra::plot(mask(WRI.Soy.CarbonLoss.ha.yr,ExtractBy()),
...
renderPlot({terra::plot(mask(WRI.Both.CarbonLoss.ha.yr,ExtractBy()),
